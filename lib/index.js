...
/**
 * Loads devproxy config
 * Firstly removes cached config module
 * Then requires a new one
 *
 * @returns {Object} - devproxy configuration object
 */
function loadConfig() {
...
function createProxy(userConfig) {
    ...
    const transformerMiddleware = function(req, res, next) {
        ...
        // transformation is handled later (within subscription to proxyRes of http-proxy module)
        if (!replaceRule || replaceRule.t) {
            return next(); // provide access to the next middleware (http-proxy in our case)
        }
    ...
    //
    // http proxy options:
    // https://github.com/http-party/node-http-proxy/blob/master/lib/http-proxy.js#L26-L42
    // or check absent options in node_modules
    ...
    // we should handle 'proxyRes' as we've set the 'selfHandledResponse' to 'true'
    proxy.on('proxyRes', (proxyRes, req, res) => {
        ...
        // fixing content-type
        // (is it possible that content-type fixing above doesn't work?)
    ...
    if (config.ssl) {
        ...
        httpsServer = https.createServer(config.ssl, httpsApp).listen(config.sslPort, 'localhost');
